...
Run:
    python app.py            # normal (SSH to real systems)
    python app.py --demo     # offline: uses bundled sample output
...
def api_run():
    ...
    def work(idx, sys_def):
        ...
        if DEMO_MODE:
            res = svc_client.run_demo(command)
        else:
...
def main():
    ...
    parser.add_argument("--demo", action="store_true",
                        help="Offline mode using bundled sample output (no SSH).")
    parser.add_argument("--port", type=int, default=5000)
    parser.add_argument("--host", default="127.0.0.1")
    args = parser.parse_args()
    DEMO_MODE = args.demo
    banner = "  [DEMO MODE — no real connections]" if DEMO_MODE else ""
    print(f" * Storage Virtualize Easy List → http://{args.host}:{args.port}{banner}")
